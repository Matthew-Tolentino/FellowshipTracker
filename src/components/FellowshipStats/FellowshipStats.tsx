...
const FellowshipStats: React.FC<IFellowshipStats> = (({fellowship}) => {
  return (
    <div className="fellowshipStats-container">
      ...
      <div className='stats'>
        <span>
          Day {Math.floor(dayDiff(fellowship.startDate, new Date()))} of {dayDiff(fellowship.startDate, fellowship.endDate).toFixed(0)}
        </span>
        <GiRing />
        <span>{fellowship.getTotalMiles().toFixed(2)} mi / {fellowship.goal} mi Traveled</span>
      </div>
    </div>
  )
})
